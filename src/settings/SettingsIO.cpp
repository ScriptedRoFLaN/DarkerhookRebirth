...
bool settings::SettingsWriter::saveTo(std::string path, bool autosave)
{
    ...
    if (!stream || stream.bad() || !stream.is_open() || stream.fail())
    {
        ...
        if (!autosave)
            g_ICvar->ConsoleColorPrintf(MENU_COLOR, "DARKERHOOK: cat_save: Can't create config file!\n");
        return false;
    }
    ...
    {
        if (!autosave)
            g_ICvar->ConsoleColorPrintf(MENU_COLOR, "DARKERHOOK: cat_save: Failed to save config!\n");
        logging::File("cat_save: FATAL! Stream bad!");
    }
    else if (!autosave)
        g_ICvar->ConsoleColorPrintf(MENU_COLOR, "DARKERHOOK: cat_save: Successfully saved config!\n");
    stream.close();
    if (stream.fail())
...
bool settings::SettingsReader::loadFrom(std::string path)
{
    ...
    if (stream.fail())
    {
        ...
        g_ICvar->ConsoleColorPrintf(MENU_COLOR, "DARKERHOOK: cat_load: File doesn't exist / can't open file!\n");
        return false;
    }
    ...
    if (stream.fail() && !stream.eof())
    {
        ...
        g_ICvar->ConsoleColorPrintf(MENU_COLOR, "DARKERHOOK: cat_load: Failed to read config!\n");
        return false;
    }

    logging::Info("cat_load: Read Success!");
    g_ICvar->ConsoleColorPrintf(MENU_COLOR, "DARKERHOOK: cat_load: Successfully loaded config!\n");
    finishString(true);
...
bool settings::SettingsReader::loadFromString(std::string stream)
{
    ...
    if (stream == "")
    {
        ...
        g_ICvar->ConsoleColorPrintf(MENU_COLOR, "DARKERHOOK: cat_load: Empty String!\n");
        return false;
    }
